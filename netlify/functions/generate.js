...
exports.handler = async (event) => {
  ...
  try {
    const { contractType, contractText, suggestions, mode } = JSON.parse(event.body);

    if (!contractText) {
      return { statusCode: 400, headers, body: JSON.stringify({ error: "缺少合同原文" }) };
    }

    if (!suggestions || !Array.isArray(suggestions) || suggestions.length === 0) {
      return { statusCode: 400, headers, body: JSON.stringify({ error: "请选择要修改的风险点" }) };
    }
    ...
    if (!response.ok) {
      return { statusCode: 502, headers, body: JSON.stringify({ error: "生成失败" }) };
    }

    const data = await response.json();
    const content = data.choices[0]?.message?.content ?? "生成失败";

    return { statusCode: 200, headers, body: JSON.stringify({ modifiedText: content }) };
  } catch (error) {
    console.error("Generate error:", error);
    return { statusCode: 500, headers, body: JSON.stringify({ error: "生成过程中出错" }) };
  }
};
